...
async def get_forecast(lat: float, lon: float, dates: list[str]) -> list[dict]:
    ...
    Each entry: date, temp_high_c, temp_low_c, description, is_clear,
                is_rainy, sunrise_iso, sunset_iso, available (bool).
    Dates beyond the 16-day forecast horizon are returned with available=false.
    """
    params = {
        "latitude":      lat,
        "longitude":     lon,
    ...
    for d in dates:
        if d in available_dates:
            ...
            def _get(key, default=None):
                arr = daily.get(key, [])
                return arr[i] if i is not None and i < len(arr) else default
            ...
            result.append({
                "date":        d,
                "temp_high_c": round(temp_max, 1) if temp_max is not None else None,
                "temp_low_c":  round(temp_min, 1) if temp_min is not None else None,
                "description": _WMO_DESCRIPTION.get(code, "Unknown"),
                "is_clear":    code in (0, 1, 2),
