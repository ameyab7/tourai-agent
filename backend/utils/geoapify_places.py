...
def _dedupe_pois(pois: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Drop duplicate POIs using Geoapify place_id as primary key.

    Falls back to (lat,lon,name) if place_id missing. Uses 5 decimal places
    (~1.1m precision) for coordinate matching.
    """
    seen: set = set()
    ...
    for poi in pois:
        ...
        if pid:
            ...
            key = (round(poi["lat"], 5), round(poi["lon"], 5), poi["name"].lower().strip())
            if key in seen:
                continue
            seen.add(key)
        result.append(poi)
