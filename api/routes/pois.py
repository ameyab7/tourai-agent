...
async def _fetch_area_buildings(lat: float, lon: float) -> dict:
    ...
    logger.info(
        f"Projected {len(projected)} building footprints at ({round(lat, 4)}, {round(lon, 4)}) — {with_utm} have UTM geometry"
    )
...
async def get_visible_pois(body: VisiblePoisRequest) -> VisiblePoisResponse:
    ...
    if cached:
        ...
        logger.info(
            f"Visible POIs at ({round(body.latitude, 5)}, {round(body.longitude, 5)}) "
            f"heading {round(body.heading, 1)}° — "
            f"{len(cached.get('visible_pois', []))}/{cached.get('total_checked', 0)} visible "
            f"on {cached.get('street_name')!r} [cache hit, {round((time.perf_counter() - t0) * 1000)}ms]"
        )
    ...
    async def _fetch_street():
        try:
            ...
            logger.warning("Street name lookup failed (OSRM unavailable) — continuing without street name")
            return None
    ...
    try:
        ...
        logger.error("POI fetch failed — returning 502 to client", extra={"exc": traceback.format_exc()})
    ...
    if needs_pois:
        ...
        if new_tall:
            logger.info(f"Added {len(new_tall)} tall buildings from Overpass (not in Geoapify)")
        pois = pois + new_tall
    ...
    buildings_n  = len(buildings) if buildings else 0
    visible_names = [p["name"] for p in result_payload["visible_pois"]]
    logger.info(
        f"Visible POIs at ({round(body.latitude, 5)}, {round(body.longitude, 5)}) "
        f"heading {round(body.heading, 1)}° — "
        f"{len(visible)}/{len(pois)} visible on {street!r} "
        f"[{buildings_n} buildings, {elapsed_ms}ms] → {visible_names}"
    )
...
async def get_current_street(
    ...
    try:
        ...
        logger.error("OSRM error on /v1/current-street — returning 502", extra={"exc": traceback.format_exc()})
