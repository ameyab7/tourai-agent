...
export default function PlanScreen() {
  ...
  async function searchLocations(query) {
    if (query.length < 2) { setLocationOpts([]); return; }
    setLocationLoading(true);
    try {
      const res = await fetch(`${API_BASE}/v1/locations/search`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setLocationOpts(data.options || []);
      } else {
        setLocationOpts([]);
      }
    } catch (_) { setLocationOpts([]); }
    setLocationLoading(false);
  }
  function onDestinationChange(text) {
    setDestination(text);
    if (locationSearchTimeout.current) clearTimeout(locationSearchTimeout.current);
    locationSearchTimeout.current = setTimeout(() => searchLocations(text), 300);
  }
  function selectLocation(opt) {
    setDestination(opt.display_name);
    setLocationOpts([]);
    setLocationLoading(false);
  }
    ...
    function processLine(line) {
      ...
      try {
        ...
        if (['stage','start','step','result'].includes(event.type)) {
          ...
          setPlan(event.plan); setPlanId(event.plan_id); setStreaming(false); setShowPartial(false); setShowReveal(true);
        } else if (event.type === 'error') {
          setError(event.message); setStreaming(false);
        } else if (event.type === 'disambiguation') {
          setDisambigOptions(event.options); setStreaming(false);
        }
      } catch (_) {}
    }
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 60 }}>
          <Text style={{ fontSize: 22, fontWeight: '600', color: '#fff', marginBottom: 8 }}>
            Which {destination}?
          </Text>
          <Text style={{ fontSize: 15, color: C.muted, marginBottom: 24 }}>
            Multiple places match your search. Pick the right one:
          </Text>
          {disambigOptions.map((opt, idx) => {
            const parts = opt.display_name.split(', ');
            const main = parts[0] || opt.display_name;
            const sub = parts.slice(1).join(', ') || `${opt.country_code}${opt.state ? `, ${opt.state}` : ''}`;
            return (
              <Pressable
                key={idx}
                style={{ backgroundColor: '#1a1f2e', borderRadius: 12, padding: 16, marginBottom: 12 }}
                onPress={() => {
                  setDestination(opt.display_name);
                  setDisambigOptions(null);
                }}
              >
                <Text style={{ fontSize: 17, color: '#fff', fontWeight: '500' }}>{main}</Text>
                <Text style={{ fontSize: 14, color: C.muted, marginTop: 4 }}>{sub}</Text>
              </Pressable>
            );
          })}
          <Pressable
            style={{ marginTop: 12 }}
            onPress={() => { setDisambigOptions(null); setError(null); }}
          >
            <Text style={{ fontSize: 15, color: C.accent, textAlign: 'center' }}>Cancel</Text>
          </Pressable>
        </ScrollView>
      </SafeAreaView>
    );
      ...
      >
        ...
        <View style={{ paddingHorizontal: 20, marginBottom: 28 }}>
          ...
          {locationOpts.length > 0 && (
            <View style={{ marginTop: 4, backgroundColor: '#fff', borderRadius: 12, marginHorizontal: 4, shadowColor: '#000', shadowOffset: {width:0,height:2}, shadowOpacity: 0.15, shadowRadius: 4, elevation: 4 }}>
              {locationOpts.map((opt, i) => (
                <Pressable key={i} style={{ paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: i < locationOpts.length-1 ? 1 : 0, borderBottomColor: '#eee' }} onPress={() => selectLocation(opt)}>
                  <Text style={{ fontSize: 15, color: C.text }}>{opt.display_name}</Text>
                </Pressable>
              ))}
            </View>
          )}
